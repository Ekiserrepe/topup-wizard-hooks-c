...
int64_t hook(uint32_t reserved) {
    ...
    if(BUFFER_EQUAL_20(activity, ACC_OUT)) {
        ...
        if (otxn_param(SVAR(amount), "A", 1) == 8) {
            // Amount provided: add or update this account in the whitelist.
            uint8_t keylet[34];
            ...
            // No amount: remove this account from the whitelist (delete its state).
            state_set(0, 0, SBUF(des_acc));
            DONE("Topup: Removed from whitelist.");
        }
    } else {
...
// cbak() recibe el resultado de las transacciones emitidas. En Xahau el WASM de
// un hook DEBE exportar tanto hook como cbak; si falta cbak el SetHook es
// rechazado con temMALFORMED. Implementación trivial (no necesitamos reaccionar).
